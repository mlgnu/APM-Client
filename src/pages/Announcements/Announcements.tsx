...
export function Announcements(props: any) {
  useEffect(() => {
  ...
  // console.log(user);
  // console.log('from user context: announcements');
  ...
  const announcements = data?.data?.announcementsPage.map((announcement: any) => (
    <Announcement
      key={announcement.id}
      id={announcement.id}
  ...
  return (
    <>
      ...
      />
     <Container size={1200}> 
      <Accordion className={classes.announcements} variant="contained">{announcements}</Accordion>;
      </Container>
      <Pagination className={classes.pagination} total={10}/>
      {user && (
